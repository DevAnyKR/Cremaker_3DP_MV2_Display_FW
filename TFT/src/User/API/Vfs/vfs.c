...
void setPrintModelIcon(bool exist) { infoFile.modelIcon = exist; }

bool isPrintModelIcon(void) { return infoFile.modelIcon; }
...
bool isRootFolder(void) { return !strchr(infoFile.path, '/'); }
...
const char *hideFilenameExtension(uint8_t index) { return hideExtension(infoFile.longFile[index] != NULL ? infoFile.longFile[index] : infoFile.file[index]); }
...
const char *restoreFilenameExtension(uint8_t index) { return restoreExtension(infoFile.longFile[index] != NULL ? infoFile.longFile[index] : infoFile.file[index]); }
...
void loopVolumeSource(void) {
  for (uint8_t i = 0; i < FF_VOLUMES; i++) {
    if (volumeSrcStatus[i] != (*volumeInserted[i])()) {
      const int16_t labelSDStates[FF_VOLUMES][2] = {{LABEL_TFT_SD_REMOVED, LABEL_TFT_SD_INSERTED}, {LABEL_TFT_USB_REMOVED, LABEL_TFT_USB_INSERTED}};
