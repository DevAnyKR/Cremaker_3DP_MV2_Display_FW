...
static inline int config_int(void) { return (strtol(&cur_line[c_index], NULL, 10)); }
...
static inline float config_float(void) { return (strtod(&cur_line[c_index], NULL)); }
...
static inline bool config_is_hex(void) { return (strstr(&cur_line[c_index], "0x") != NULL); }

// get the hex after config keyword
static inline uint32_t config_hex(void) { return (strtol(&cur_line[c_index], NULL, 16)); }
...
static void parseConfigKey(uint16_t index) {
  ...
  case C_INDEX_SERIAL_PORT:
    ...
    if (key_seen("P2:"))
      SET_VALID_INT_VALUE(infoSettings.serial_port[1], 0, BAUDRATE_COUNT - 1);
    if (key_seen("P3:"))
      SET_VALID_INT_VALUE(infoSettings.serial_port[2], 0, BAUDRATE_COUNT - 1);
    if (key_seen("P4:"))
      SET_VALID_INT_VALUE(infoSettings.serial_port[3], 0, BAUDRATE_COUNT - 1);
    break;
  ...
  case C_INDEX_MAX_TEMP:
    if (key_seen("T0:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[NOZZLE0], MIN_TOOL_TEMP, MAX_TOOL_TEMP);
    if (key_seen("T1:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[NOZZLE1], MIN_TOOL_TEMP, MAX_TOOL_TEMP);
    if (key_seen("T2:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[NOZZLE2], MIN_TOOL_TEMP, MAX_TOOL_TEMP);
    if (key_seen("T3:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[NOZZLE3], MIN_TOOL_TEMP, MAX_TOOL_TEMP);
    if (key_seen("T4:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[NOZZLE4], MIN_TOOL_TEMP, MAX_TOOL_TEMP);
    if (key_seen("T5:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[NOZZLE5], MIN_TOOL_TEMP, MAX_TOOL_TEMP);
    if (key_seen("BED:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[BED], MIN_BED_TEMP, MAX_BED_TEMP);
    if (key_seen("CHAMBER:"))
      SET_VALID_INT_VALUE(infoSettings.max_temp[CHAMBER], MIN_CHAMBER_TEMP, MAX_CHAMBER_TEMP);
    break;
  ...
  case C_INDEX_FAN_MAX:
    if (key_seen("F0:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[0], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("F1:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[1], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("F2:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[2], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("F3:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[3], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("F4:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[4], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("F5:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[5], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("CtA:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[6], MIN_FAN_SPEED, MAX_FAN_SPEED);
    if (key_seen("CtI:"))
      SET_VALID_INT_VALUE(infoSettings.fan_max[7], MIN_FAN_SPEED, MAX_FAN_SPEED);
    break;

  case C_INDEX_SIZE_MIN:
    if (key_seen("X"))
      SET_VALID_INT_VALUE(infoSettings.machine_size_min[X_AXIS], MIN_SIZE_LIMIT, MAX_SIZE_LIMIT);
    if (key_seen("Y"))
      SET_VALID_INT_VALUE(infoSettings.machine_size_min[Y_AXIS], MIN_SIZE_LIMIT, MAX_SIZE_LIMIT);
    if (key_seen("Z"))
      SET_VALID_INT_VALUE(infoSettings.machine_size_min[Z_AXIS], MIN_SIZE_LIMIT, MAX_SIZE_LIMIT);
    break;

  case C_INDEX_SIZE_MAX:
    if (key_seen("X"))
      SET_VALID_INT_VALUE(infoSettings.machine_size_max[X_AXIS], MIN_SIZE_LIMIT, MAX_SIZE_LIMIT);
    if (key_seen("Y"))
      SET_VALID_INT_VALUE(infoSettings.machine_size_max[Y_AXIS], MIN_SIZE_LIMIT, MAX_SIZE_LIMIT);
    if (key_seen("Z"))
      SET_VALID_INT_VALUE(infoSettings.machine_size_max[Z_AXIS], MIN_SIZE_LIMIT, MAX_SIZE_LIMIT);
    break;

  case C_INDEX_XY_SPEED:
    if (key_seen("S"))
      SET_VALID_INT_VALUE(infoSettings.xy_speed[0], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("N"))
      SET_VALID_INT_VALUE(infoSettings.xy_speed[1], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("F"))
      SET_VALID_INT_VALUE(infoSettings.xy_speed[2], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    break;

  case C_INDEX_Z_SPEED:
    if (key_seen("S"))
      SET_VALID_INT_VALUE(infoSettings.z_speed[0], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("N"))
      SET_VALID_INT_VALUE(infoSettings.z_speed[1], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("F"))
      SET_VALID_INT_VALUE(infoSettings.z_speed[2], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    break;

  case C_INDEX_EXT_SPEED:
    if (key_seen("S"))
      SET_VALID_INT_VALUE(infoSettings.ext_speed[0], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("N"))
      SET_VALID_INT_VALUE(infoSettings.ext_speed[1], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("F"))
      SET_VALID_INT_VALUE(infoSettings.ext_speed[2], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    break;
  ...
  case C_INDEX_PAUSE_RETRACT:
    if (key_seen("R"))
      SET_VALID_FLOAT_VALUE(infoSettings.pause_retract_len, MIN_RETRACT_LIMIT, MAX_RETRACT_LIMIT);
    if (key_seen("P"))
      SET_VALID_FLOAT_VALUE(infoSettings.resume_purge_len, MIN_RETRACT_LIMIT, MAX_RETRACT_LIMIT);
    break;

  case C_INDEX_PAUSE_POS:
    if (key_seen("X"))
      SET_VALID_FLOAT_VALUE(infoSettings.pause_pos[X_AXIS], MIN_XY_POS_LIMIT, MAX_SIZE_LIMIT);
    if (key_seen("Y"))
      SET_VALID_FLOAT_VALUE(infoSettings.pause_pos[Y_AXIS], MIN_XY_POS_LIMIT, MAX_SIZE_LIMIT);
    break;
  ...
  case C_INDEX_PAUSE_FEEDRATE:
    if (key_seen("XY"))
      SET_VALID_INT_VALUE(infoSettings.pause_feedrate[FEEDRATE_XY], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("Z"))
      SET_VALID_INT_VALUE(infoSettings.pause_feedrate[FEEDRATE_Z], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("E"))
      SET_VALID_INT_VALUE(infoSettings.pause_feedrate[FEEDRATE_E], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    break;
  ...
  case C_INDEX_LEVEL_FEEDRATE:
    if (key_seen("XY"))
      SET_VALID_INT_VALUE(infoSettings.level_feedrate[FEEDRATE_XY], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    if (key_seen("Z"))
      SET_VALID_INT_VALUE(infoSettings.level_feedrate[FEEDRATE_Z], MIN_SPEED_LIMIT, MAX_SPEED_LIMIT);
    break;

  case C_INDEX_INVERTED_AXIS:
    if (key_seen("X"))
      SET_BIT_VALUE(infoSettings.inverted_axis, X_AXIS, getOnOff());
    if (key_seen("Y"))
      SET_BIT_VALUE(infoSettings.inverted_axis, Y_AXIS, getOnOff());
    if (key_seen("Z"))
      SET_BIT_VALUE(infoSettings.inverted_axis, Z_AXIS, getOnOff());

    // leveling Y axis (E_AXIS -> index for param "inverted_axis LY<x>" in "config.ini")
    if (key_seen("LY"))
      SET_BIT_VALUE(infoSettings.inverted_axis, E_AXIS, getOnOff());
    break;
  ...
  case C_INDEX_PREHEAT_TEMP_6: {
    ...
    if (key_seen("B"))
      SET_VALID_INT_VALUE(configPreheatStore->preheat_bed[val_index], MIN_BED_TEMP, MAX_BED_TEMP);
    break;
  }
  ...
  case C_INDEX_LED_COLOR:
    if (key_seen("R:"))
      SET_VALID_INT_VALUE(infoSettings.led_color[0], MIN_LED_COLOR_COMP, MAX_LED_COLOR_COMP);
    if (key_seen("G:"))
      SET_VALID_INT_VALUE(infoSettings.led_color[1], MIN_LED_COLOR_COMP, MAX_LED_COLOR_COMP);
    if (key_seen("B:"))
      SET_VALID_INT_VALUE(infoSettings.led_color[2], MIN_LED_COLOR_COMP, MAX_LED_COLOR_COMP);
    if (key_seen("W:"))
      SET_VALID_INT_VALUE(infoSettings.led_color[3], MIN_LED_COLOR_COMP, MAX_LED_COLOR_COMP);
    if (key_seen("P:"))
      SET_VALID_INT_VALUE(infoSettings.led_color[4], MIN_LED_COLOR_COMP, MAX_LED_COLOR_COMP);
    if (key_seen("I:"))
      SET_VALID_INT_VALUE(infoSettings.led_color[5], MIN_LED_COLOR_COMP, MAX_LED_COLOR_COMP);
    break;
...
#ifdef CONFIG_DEBUG
      GUI_DispStringInRect(recterrortxt.x0, recterrortxt.y0 + (BYTE_HEIGHT * 2), recterrortxt.x1, recterrortxt.y1, (uint8_t *)configPrintGcodes->start_gcode);
      Delay_ms(1000);
      Delay_ms(1000);
#endif
    }
...
#ifdef CONFIG_DEBUG
      GUI_DispStringInRect(recterrortxt.x0, recterrortxt.y0 + (BYTE_HEIGHT * 2), recterrortxt.x1, recterrortxt.y1, (uint8_t *)configPrintGcodes->end_gcode);
      Delay_ms(1000);
      Delay_ms(1000);
#endif
    }
...
#ifdef CONFIG_DEBUG
      GUI_DispStringInRect(recterrortxt.x0, recterrortxt.y0 + (BYTE_HEIGHT * 2), recterrortxt.x1, recterrortxt.y1, (uint8_t *)configPrintGcodes->cancel_gcode);
      Delay_ms(1000);
      Delay_ms(1000);
#endif
    }
