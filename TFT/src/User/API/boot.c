...
typedef enum { BMP_SUCCESS, BMP_NOTFOUND, BMP_NOT24BIT, BMP_INVALIDFILE } BMPUPDATE_STAT;
...
static const uint32_t fontAddrList[]   = {BYTE_ASCII_ADDR, WORD_UNICODE_ADDR, LARGE_FONT_ADDR, _8X16_FONT_ADDR};

static const char *fontPathList[]      = {FONT_UPDATE_DIR "/" FILE_ASCII_FONT, FONT_UPDATE_DIR "/" FILE_UNICODE_FONT, FONT_UPDATE_DIR "/" FILE_LARGE_ASCII_FONT, FONT_UPDATE_DIR "/" FILE_8X16_ASCII_FONT};
...
static inline bool updateIcon(char *rootDir) {
  ...
  for (int i = 0; i < COUNT(iconBmpName); i++) {
    ...
    if (bmpState == BMP_SUCCESS) { // display bmp update success
      found++;
      IMAGE_ReadDisplay(iconUpdateRect.x0, iconUpdateRect.y0, ICON_ADDR(i));
    } else { // display bmp update fail
      notFound++;
      dispIconFail((uint8_t *)curBmpPath, bmpState);
    }
...
static inline void scanRenameUpdate(char *rootDir) {
  ...
  if (f_dir_exists(curPath)) { // UPDATE_DIR exists
