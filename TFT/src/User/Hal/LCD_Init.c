...
uint32_t LCD_ReadPixel_24Bit(int16_t x, int16_t y) { return pLCD_ReadPixel_24Bit(x, y); }
...
static inline void LCD_Init_Sequential(void) {
  ...
  if (0) {
    ...
    RM68042_Init_Sequential();
    pLCD_SetDirection = RM68042_SetDirection pLCD_SetWindow = RM68042_SetWindow;
#ifdef SCREEN_SHOT_TO_SD
    pLCD_ReadPixel_24Bit = RM68042_ReadPixel_24Bit;
#endif
  }
...
void LCD_RefreshDirection(uint8_t rotate) { pLCD_SetDirection(rotate); }

void LCD_SetWindow(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey) { pLCD_SetWindow(sx, sy, ex, ey); }
